...
class SecureVisionPipeline:
    ...
    def process_frame(self, frame, frame_number, capture_callback=None):
        ...
        for event in fight_events:
            ...
            vel = event.get('velocity', 0.0)
            pose = event.get('pose', 0.0)
            fight_status_map[id1] = {'status': status, 'partner': id2, 'timer': timer, 'velocity': vel, 'pose': pose}
            fight_status_map[id2] = {'status': status, 'partner': id1, 'timer': timer, 'velocity': vel, 'pose': pose}
            ...
            if status == 'CONFIRMED':
                ...
                if self.fight_snapshot_cooldown == 0:
                    ...
                    # LOG DB EVENT
                    self.stats_manager.log_event("FIGHT", {"ids": f"{id1}-{id2}", "stream": self.stream_id, "velocity": vel, "pose": pose})
                
        # 3.1. Frame Recording Logic
        if self.recording_frames_left > 0:
            if capture_callback and hasattr(self, '_last_critical_reason'):
                capture_callback(frame.copy(), getattr(self, '_last_critical_reason', "CRITICAL EVENT"))
                
            self.recording_frames_left -= 1
            if self.recording_frames_left == 0:
                logger.info(f"[{self.stream_id}] Incident Recording Snapshot Saved.")
        
        # 3.2. ReID Processing (Person Identification) - DISABLED
        ...
        # Check if any person is present in the current frame or recently ghosted
        person_present = any(t['class'] == 'person' and (frame_number - t['last_seen'] <= GHOST_FRAMES_PERSON) for t in all_tracks.values())
        ...
        for tid, track in all_tracks.items():
            ...
            display_cls = "Luggage" if cls in LUGGAGE_CLASSES else cls
            ...
            if cls in WEAPON_CLASSES:
                if not person_present:
                    continue # Do not flag or count weapons unless a person is in the scene
            ...
            if tid in fight_status_map:
                ...
                vel = f_info.get('velocity', 0.0)
                pose = f_info.get('pose', 0.0)
                if f_info['status'] == 'CONFIRMED':
                    obj_data['status'] = 'CRITICAL'
                    obj_data['details'] = f"FIGHTING with {f_info['partner']} (V:{vel:.1f} P:{pose:.0f})"
                    current_color = (139, 0, 0)  # Dark Blue in BGR
                    label_suffix += " (FIGHT!)"
                elif f_info['status'] == 'WARNING':
                     obj_data['status'] = 'WARNING'
                     obj_data['details'] = f"Analyzing {timer_val}/30 (V:{vel:.1f} P:{pose:.0f})"
            ...
            should_draw = False
            if (obj_data['category'] in LUGGAGE_CLASSES or obj_data['category'] == 'Luggage') and obj_data['status'] == 'CRITICAL':
            ...
            if should_draw:
                ...
                label = f"{display_cls.upper()} {display_id}{label_suffix}"
